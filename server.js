...
class MQTTPublisher {
    ...
    publishMessage() {
        ...
        for (let i = 0; i < samples_per_channel; i++) {
            all_channel_data[this.channel - 2][i] = this.frequency;
        }
            ...
            try {
                this.client.publish(topic, buffer, { qos: 1 }, (err) => {
                    if (err) {
                        ...
                        console.log(`${new Date().toISOString()} - INFO - [${this.count}] Published to ${topic}: frame ${this.frame_index} with ${message_values.length} values`);
                    }
                });
            } catch (e) {
...
if (require.main === module) {
    const broker = '192.168.1.231';
    const topics = ['sarayu/d1/topic1'];
    const publisher = new MQTTPublisher(broker, topics);
}
